...
write.table(row.cluster$labels , file =  paste0(Pathtosave,"_row.cluster$Labels_.tsv"), sep="\t", quote = FALSE, row.names = FALSE, col.names = FALSE) # saving the original rows
write.table( row.cluster$labels[row.cluster$order] , file =  paste0(Pathtosave,"_row.cluster$Labels[row.cluster$]_.tsv"), sep="\t", quote = FALSE , row.names = FALSE, col.names = FALSE) # saving the clustered rows
saveRDS( row.cluster,file=paste0(Pathtosave,"_row.cluster_.RDS")) # saving the object of clustered rows

col.distance = dist(t(MyMatrix), method = "euclidean")
col.cluster = hclust(col.distance, method = "ward.D2")
write.table(col.cluster$labels , file =  paste0(Pathtosave,"_col.cluster$Labels_.tsv"), sep="\t", quote = FALSE, row.names = FALSE, col.names = FALSE) # saving the original column order
write.table( col.cluster$labels[col.cluster$order] , file =  paste0(Pathtosave,"_col.cluster$Labels[col.cluster$]_.tsv"), sep="\t", quote = FALSE , row.names = FALSE, col.names = FALSE) # saving the clustered columns
saveRDS( row.cluster,file=paste0(Pathtosave,"_col.cluster_.RDS")) # saving the object of clustered columns
